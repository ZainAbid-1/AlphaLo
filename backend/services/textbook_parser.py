...
class TextbookIngestor:
    ...
    def vectorization(self, chunks):
        print(f"🚀 Starting embedding & Pinecone upload for {len(chunks)} chunks... (This WILL take several minutes for a full textbook!)")
        ...
        for i, start_idx in enumerate(range(0, len(chunks), batch_size)):
            ...
            print(f"⏳ Uploading batch {i + 1}/{total_batches} ({len(batch)} chunks)...")
            if vectors is None:
